...
public class ReportId extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String thisBoardCode = request.getParameter("thisBoardCode");
...
//		String uploadPath = this.getServletContext().getRealPath("/images/saveFileDerectory/");//실제 회사에선 서버에 올리기 때문에 이 방법이 맞으나 지금은 로컬 임시폴더에 저장됨
		...
		out.println("<script>alert('신고처리가 완료되었습니다.'); location.href='" + request.getContextPath() + "/main/board.jsp?post_bcode="+thisBoardCode+"'; </script>");
		//out.println("</head><body></body></html>");
		//response.sendRedirect(request.getContextPath()+"/main/board.jsp?post_bcode="+thisBoardCode);
