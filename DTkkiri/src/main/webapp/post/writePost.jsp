...
<%
	...
	//현재게시판코드 가져오기
	String thisBcode=request.getParameter("thisBcode");
%>
<meta charset="UTF-8">
	    ...
	    <!-- 아바타카드 나오게하고 안에 버튼으로 마이페이지 이동할 수 있게 하기 -->
	        ...
	        <li class="nav-item dropdown">
	            ...
	            <ul id="ulBox" class="dropdown-menu dropdown-menu-end" aria-labelledby="navbarDropdown" style="border-radius:20px;   background-color: rgba(255,255,255,1);"> 
						    ...
						    <br>
						    <br>
						    <a href="../memberUpdate/memUpdate.jsp" class="btn btn-primary">내정보수정</a>
						    <a href="#" class="btn btn-primary" id="out">회원탈퇴</a>
						
						    <%
						    	if(pid.equals("STUDENT")){
						    %>
                            <a href="" class="btn btn-primary"  style="display: none;">회원관리</a>
                            <%
						    	}else if(pid.equals("MAINADMIN")){
                            %>
                            <a href="../adminView/adminView.jsp" class="btn btn-primary" >회원관리</a>
                            <%
						    	}else if(pid.equals("SUBADMIN")){
                            %>
                            <a href="../adminView/subAdminView.jsp" class="btn btn-primary">회원관리</a>	                            	
                            <%
						    	}
                            %>
						    <a href="<%=request.getContextPath() %>/sessionLogout.do" class="btn btn-primary">로그아웃</a>
						    </p> 
					  	</div>
	            </ul>
	        </li>
        ...
        <div id="layoutSidenav_content">
            ...
            %>
				<div class="container">
					<br>
					<br>
					<h2 style="font-family: yg-jalnan"><img src="/DTkkiri/images/연필.png"> 게시글 작성</h2>
					<hr id="hrStyle">
					<br>
					<form action="/DTkkiri/registPost.do" method="post" enctype="multipart/form-data">
						...
						<div class="form-group">
							...
							<textarea class="form-control" rows="3" cols="" placeholder="내용을 입력하세요." name="content"></textarea>
						</div>
						<div class="form-group">
							<label for="file" style="font-family: yg-jalnan">파일첨부</label> 
							<input type="file" id="file" name="files" multiple="multiple" style="font-family: yg-jalnan">
						</div>
						<input id="hnum1" type="hidden" name="boardCode" value="<%=post_bcode%>">
						<input id="hnum2" type="hidden" name="id" value="<%=memVo.getMem_id()%>">
						<button type="submit" class="btn btn-warning" style="float:right;font-family: yg-jalnan">작성</button>
						<button type="button" class="btn btn-warning" onclick="location.href='../main/board.jsp?post_bcode='+document.getElementById('hnum1').value" style="float:right;font-family: yg-jalnan">돌아가기</button>
					</form>
				</div>
			<%
            	}else{
			%>
				<div class="container">
					...
					<br>
					<h2>신고글 작성</h2>
					<form action="/DTkkiri/reportId.do" method="post" enctype="multipart/form-data">
						<div class="form-group">
							<label for="title">제목:</label> 
							<input type="text" class="form-control" id="title" value="신고 ID : <%=reportId %>" name="title" readonly>
						</div>
						<div class="form-group">
						...
						<div class="form-group">
							<label for="file" >증빙자료첨부(Optional):</label> 
							<input type="file" id="file" name="files" multiple="multiple">
						</div>
						<input id="hnum1" type="hidden" name="boardCode" value="<%=post_bcode%>">
						<input id="hnum2" type="hidden" name="id" value="<%=memVo.getMem_id()%>">
						<input id="hnum3" type="hidden" name="thisBoardCode" value="<%=thisBcode%>">
						<button type="submit" class="btn btn-warning" style="float:right;">작성</button>
						<button type="button" class="btn btn-warning" onclick="location.href='../main/board.jsp?post_bcode='+document.getElementById('hnum1').value" style="float:right;">돌아가기</button>
					</form>
				</div>
			<%
            	}
